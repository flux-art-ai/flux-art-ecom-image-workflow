...
“商品看起来更宽、更短或一端更大”可能来自拍摄距离、机位和透视，也可能是生成或编辑改变了结构。先用同一完整 SKU 的事实证据诊断，不能凭单张斜拍图决定真实比例。

### 建立四项对照

1. **正视基线**：准备正面和侧面尽量平直的清楚照片，标明拍摄面，不混用其他 SKU 或包装版本。
2. **尺寸来源**：记录真实长、宽、高或关键部件尺寸及来源；尺寸不确定时暂停，不从图片反推精确数值。
3. **尺度参照**：需要展示大小关系时，使用尺寸已知且与商品处在同一平面的参照；远近位置不同的物体不能直接比较。
4. **拍摄记录**：保留原始图、相机距离、机位方向与裁切范围。没有可靠记录时，用稳定机位重拍一组可比照片。

| 观察 | 判断 | 下一步 | 停止条件 |
|---|---|---|---|
| 正视图与尺寸表一致，只有近距离斜拍前大后小 | 拍摄透视问题 | 优先调整拍摄距离与机位重拍；有真实基线后，可用[产品精修](https://flux-art.cn/zh/ai-ecommerce/product-retouch)处理已确认的透视变形 | 修正带动轮廓、接口、标签或包装文字变化时回退 |
| 多个角度中同一部件都与尺寸表冲突 | 结构错误 | 回到真实素材或最后通过母版；需要重建产品图时评估 [GPT Image 2](https://flux-art.cn/zh/models/gpt-image-2) | 不在错误图上继续拉伸或补画结构 |
| 正视图、尺寸表或尺度参照互相矛盾 | 证据不足 | 暂停编辑，由商品负责人确认正确 SKU 和尺寸来源后再重拍 | 未取得一致证据前不生成“正确比例”候选 |
| 结构正确，只需限制画面内的视角变化 | 有界编辑 | 从原图或最后通过版本开始；可比较 [GPT Image 2.5](https://flux-art.cn/zh/models/gpt-image-2-5)或 [Nano Banana 2](https://flux-art.cn/zh/models/nano-banana-2)，一次只改透视或构图 | 任一未要求变化区域发生偏移时停止并回退 |

限定修正提示词：

```text
仅修正拍摄造成的透视变形，使商品正面边线与已提供的正视参考一致。保持真实长宽比例、接口数量与位置、Logo、标签文字、颜色、材质、背景和阴影不变；不要拉长、压扁或新增商品结构。
```

验收时把新图、正视图、侧视图和尺寸表并排，在相同显示尺度下逐项检查轮廓、关键部件位置和尺度关系。需要展示多角度或尺寸规格时，可在[商品套图](https://flux-art.cn/zh/ai-ecommerce/product-suite)中组织对应模块，但尺寸数字仍必须来自真实资料。
