...
## 商品颜色异常：色偏、显示差异还是 SKU 换色

看到“图片颜色和实物不一样”时，先暂停批量扩图。用同一件实物、批准色卡、完整 SKU 和中性光线下的照片建立基线；不要直接吸取屏幕颜色，也不要把生成图当作标准色。

1. **复核拍摄条件**：让灰卡或色卡与商品处在同一光线，检查混合光、自动白平衡、曝光和环境反射。无法稳定复现时先重拍。
2. **复核显示条件**：在团队约定的可靠显示环境中查看原文件。若只有一台屏幕异常，记录设备、显示模式和亮度，不为单一设备修改已验收母版。
3. **核对商品版本**：确认参考实物、包装和色卡属于同一完整 SKU。不同批次或颜色 SKU 不得混作一套基线。
4. **选择最小动作**：拍摄造成的已确认色偏可有限校正；真实在售的另一种颜色走 SKU 换色；证据冲突时暂停。

| 判断 | 可执行动作 | 停止条件 |
|---|---|---|
| 原图白色或中性区域整体偏暖、偏冷，实物与批准色卡一致 | 先按真实基线校正白平衡；需要 AI 辅助时可进入[产品精修](https://flux-art.cn/zh/ai-ecommerce/product-retouch)，只处理已确认的色偏或光影 | 色卡与实物不在同一光线、材质高光被抹平，或校色带动 Logo、标签、轮廓变化 |
| 已验收文件在多数可靠设备一致，仅某一显示器偏色 | 保留母版，校验该设备的显示模式、亮度和色彩配置 | 没有证据证明源文件错误时，不重新生成商品 |
| 目标是另一个真实在售颜色 SKU | 准备目标 SKU 实物图、批准色卡和 SKU 映射，再使用[产品换色](https://flux-art.cn/zh/ai-ecommerce/product-recolor)制作候选 | 目标色没有实物依据、不同 SKU 资料冲突，或结果改变材质、包装与结构 |
| 无法确认真实颜色，或实物、色卡、包装资料互相矛盾 | 暂停并交由商品负责人确认；必要时按固定光线重拍 | 不用提示词或屏幕取色猜测商品事实 |

针对已确认的拍摄色偏，可以从最后通过母版做一次限定编辑：

```text
仅校正整张商品照片因拍摄白平衡造成的偏黄，使商品颜色与同光线下的批准色卡一致；保持商品轮廓、材质纹理、高光、Logo、标签文字、背景和阴影结构不变，不创建新的颜色版本。
```

输出后并排查看实物基线、旧图和新图，检查目标颜色、材质高光、包装文字、Logo、阴影及所有未要求变化的区域。只有目标偏差消失且没有新增变化时才建立新母版；真实 SKU 换色则必须按目标 SKU 单独验收。
