...
详情页中的包装文字、数字和参数必须有唯一事实来源。不要只把商品图交给模型后依赖图片识别，也不要让模型补全被遮挡或看不清的内容。建议在制作前建立以下资料表：

| 字段 | 唯一事实来源 | 制作时怎么用 | 发布前怎么验收 |
|---|---|---|---|
| 商品名称与型号 | 已批准的商品档案 | 原文写入提示词或后期排版，不改写型号 | 逐字对照，检查大小写、空格与连字符 |
| 包装可见文字 | 实物包装或批准版包装稿 | 标出必须保留、允许删除和不得生成的区域 | 放大检查错字、重复字与模型新增小字 |
| 规格、容量与单位 | 当前 SKU 资料 | 把数字与单位作为一个不可拆分字段 | 同时核对数值、单位和对应 SKU |
| 卖点与适用范围 | 已审核文案 | 只使用能够验证的表述，不扩写效果 | 核对原意、限制条件和当前页面规则 |
