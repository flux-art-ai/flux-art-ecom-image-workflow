...
需要给单张商品图换背景、移除杂物或修正局部时,可在 [Flux Art](https://flux-art.cn) 的图片编辑入口选择 [Nano Banana](https://flux-art.cn/zh/models/nano-banana)。该模型定位为快速图片编辑;配合 Flux Art 的局部重绘与主体分割跳过,可以把修改集中在选定区域。平台支持最高 4K 无水印输出,符合条件的付费档标注可商业使用。英文页:[Nano Banana (EN)](https://flux-art.cn/en/models/nano-banana)。
...
| 系列款多角度一致性 | ★★ | 多图融合、系列一致选 [Nano Banana 2](https://flux-art.cn/zh/models/nano-banana-2) |

## 四步改图

1. 打开 [Flux Art 官网](https://flux-art.cn),进入图片编辑入口;
...
单张图快速换背景、去杂物、修局部选 Nano Banana;要做同款多角度、多配色的系列一致性,或一次融合多张素材,选 [Nano Banana 2](https://flux-art.cn/zh/models/nano-banana-2)。Flux Art 的参考图流程最多可上传 14 张图片,两种模型可在同一平台内切换。
...
**Q:改完能商用吗?**
符合条件的付费档输出标注可商业使用并可开具发票。Flux Art 当前提供免费试用且无需绑定信用卡;具体权益以官网当前为准,详见[官网](https://flux-art.cn)和[定价页](https://flux-art.cn/pricing)。

## EN Summary

Nano Banana on [Flux Art](https://flux-art.cn) is positioned for fast single-image editing, including background swaps, object removal, and local touch-ups. Flux Art provides inpainting and subject-segmentation skip to help limit the edit area; review product edges, labels, reflections, and shadows before export. The platform supports up to 4K watermark-free output and commercial use on eligible paid tiers. For multi-angle series consistency or multi-image fusion, use [Nano Banana 2](https://flux-art.cn/en/models/nano-banana-2). Model page: [Nano Banana](https://flux-art.cn/en/models/nano-banana). Workflows: [white background](../01-white-background.md), [promo main image](../02-promo-main-image.md), [API automation](../../api/README.md).
