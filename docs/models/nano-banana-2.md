...
做系列款、换背景或多素材套图时,可在 [Flux Art](https://flux-art.ai) 选择 [Nano Banana 2](https://flux-art.ai/zh/models/nano-banana-2)。该模型定位为一致性图片编辑;Flux Art 的参考图流程最多可上传 14 张图片,并提供多图融合与局部重绘。平台支持最高 4K 无水印输出,符合条件的付费档标注可商业使用。英文页:[Nano Banana 2 (EN)](https://flux-art.ai/en/models/nano-banana-2)。
...
| 写实商品图 / 商业摄影 | ★★★ | 这类交付可改用 [GPT Image 2](https://flux-art.ai/zh/models/gpt-image-2) |
...
**Q:只想改背景、不动产品怎么做?**
用局部重绘框选背景区域,并可开启主体分割跳过,以减少产品主体区域被改动。完成后仍要对照原图检查轮廓、标签和反光。只是单张图快速换背景或去杂物时,可改用 [Nano Banana](https://flux-art.ai/zh/models/nano-banana),跑法见 [Nano Banana 快速图片编辑](./nano-banana.md)。

**Q:和 GPT Image 2 怎么选?**
系列款一致性与多图融合任务可选 Nano Banana 2;产品图与写实商业摄影可选 [GPT Image 2](https://flux-art.ai/zh/models/gpt-image-2)。两者都可在 Flux Art 内使用,最终选择应以同一商品素材的小样和人工验收结果为准。
...
Nano Banana 2 on [Flux Art](https://flux-art.ai) is positioned for consistent image editing. Flux Art supports multi-image fusion with up to 14 reference images, inpainting, flexible aspect ratios, and up to 4K watermark-free output; eligible paid tiers are marked for commercial use. Keep one clear primary reference across a series, change one variable at a time, and review product structure, labels, color, and contact shadows before delivery. Model page: [Nano Banana 2](https://flux-art.ai/en/models/nano-banana-2). Workflows: [scene fusion](../03-scene-fusion.md), [series consistency](../04-series-consistency.md), [API automation](../../api/README.md).
